import apiUrl from "../../constants/apiUrl";
import * as types from "./../types";

export const toggleDiseaseDialog = (status) => ({
    type: types.TOGGLE_DISEASE_DIALOG,
    payload: status,
});

export const fetchDiseases =
    (page = 0) =>
    async (dispatch) => {
        await fetch(apiUrl.diseases.index + `?offset=${page}&size=10`, {
            method: "GET",
        })
            .then((res) => res.json())
            .then((res) => {
                console.log(res);
                dispatch({
                    type: types.FETCH_DISEASES,
                    payload: res,
                });
            })
            .catch((err) => console.log(err));
    };

export const fetchDoctor = () => async (dispatch) => {
    await fetch(apiUrl.doctors.index, {
        method: "GET",
    })
        .then((res) => res.json())
        .then((res) => console.log(res))
        .catch((err) => console.log(err));
};

export const createDisease =
    (data, cb = () => {}) =>
    async (dispatch) => {
        await fetch(apiUrl.diseases.create, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify(data),
        })
            .then((res) => res.json())
            .then((res) => {
                console.log(res);
                if (Object.keys(res).length > 0) {
                    dispatch({
                        type: types.CREATE_DISEASE,
                        payload: res,
                    });
                    cb();
                }
            })
            .catch((err) => console.log(err));
    };

export const updateDoctor = () => async (dispatch) => {
    await fetch(apiUrl.doctors.index, {
        method: "GET",
    })
        .then((res) => res.json())
        .then((res) => console.log(res))
        .catch((err) => console.log(err));
};

export const deleteDisease = (id) => async (dispatch) => {
    await fetch(apiUrl.diseases.delete + `?id=${id}`, {
        method: "DELETE",
    })
        .then((res) => res.json())
        .then((res) => {
            console.log(res);
            if (res) {
                dispatch({
                    type: types.DELETE_DISEASE,
                    payload: id,
                });
            }
        })
        .catch((err) => console.log(err));
};
